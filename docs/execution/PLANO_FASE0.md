...
| 0.2 Puzzles mobile (+0.6-D) | ✅ Código completo — ver §12 | `feature/puzzles-mobile-ui` |
...
- **Migrations 0007/0008**: schema + seed reversível. **Seed uniforme 1500/350/0.06 para todos os perfis, existentes e novos** (decisão do PM 2026-07-12 na revisão do PR: o Elo antigo não é herdado); games do histórico herdados pelo blitz (preserva quem já saiu do provisório). Testadas localmente com dados: forward → verificação → reverse → forward, sem perda.
...
- Build `949fb931-f16f-49d3-8bda-40366cfdf8c2` (commit c446981, versão/runtime 1.1.0), perfil `preview`, ~95 min de fila+build — primeiro APK com **tudo da Fase 0 até o 0.1**: Glicko-2 por modalidade (0.3), onboarding em 3 toques (0.4), assinaturas Stripe + gating de partidas (0.1), empate/desistir (§7) e gating pré-jogo (commit 5ba4559). Entregue ao fundador para teste manual no dispositivo.
- Runtime version 1.1.0 reinicia o canal de OTA updates (o runtime 1.0.0 anterior não recebe updates deste canal) — esperado, ainda sem usuários em produção.

## 12. Item 0.2 fechado — Puzzles no mobile + 0.6-D (2026-07-13)

Executado conforme o desenho do §2, na branch `feature/puzzles-mobile-ui`. Último item de código da Fase 0.

- **Backend — streak em `PuzzleStatsView`** (sem migration): dias consecutivos com puzzle resolvido via `solved_at`, tolerando o dia corrente ainda sem solve (streak de ontem só quebra quando o dia termina). `stats/` também expõe `daily_puzzle_limit`/`remaining_puzzles_today` (None = plano pago).
- **Gating 3 puzzles/dia do plano Grátis (RF-MON-05)**: `can_solve_puzzle`/`FREE_DAILY_PUZZLE_LIMIT` em `apps/payments/access.py` (reaproveita `has_paid_access`, como previsto). **Conta puzzles RESOLVIDOS no dia** (tentativa falha não consome cota; re-resolver puzzle antigo também não — `solved_at` é o único carimbo confiável). Pré-gate no `next/` (403 `daily_limit_reached`) + defesa em profundidade no `progress/` (só solve novo é bloqueado), espelhando o padrão do `AiGameResultView`.
- **Mobile — `screen/puzzles/PuzzleScreen.tsx`** (state-based via `home.tsx`, `ActiveScreen: "puzzles"`): tabuleiro `react-native-chessboard` + chess.js validando lance-a-lance contra `solution` (UCI, com promoção), lance errado é desfeito no tabuleiro (`resetBoard(fen)`) e soma tentativa, resposta automática do "oponente" nos lances ímpares. Dificuldade adaptativa pelo rating Glicko-2 blitz (<1000 easy, 1000–1400 medium, >1400 hard) em `services/puzzles.ts`.
- **0.6-D completo**: streak em `colors.accent` no header e no card de recompensa pós-solve; bloqueio elegante ao atingir o limite com contador e CTA dourado "Assinar Premium" → SubscriptionScreen. Números sempre em `colors.text` (regra WCAG do 0.6-A).
